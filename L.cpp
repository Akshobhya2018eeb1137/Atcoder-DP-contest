...
int main()
{
	...
	for(ll gap = 0;gap< n;gap++)
	{
		for(ll i = 0;i + gap < n;i++)
		{
			if(gap == 0)
			{
				dp[i][i][0] = a[i];//0 means taro 1 means jiro
				dp[i][i][1] = -a[i];
			}
			else
			{
				dp[i][i + gap][0] = max(a[i] + dp[i + 1][i + gap][1], a[i + gap] + dp[i][i + gap -1][1]);
				dp[i][i + gap][1] = min(-a[i] + dp[i + 1][i + gap][0], -a[i + gap] + dp[i][i + gap - 1][0]); 
			}
		}
	}
	cout<<dp[0][n - 1][0];
