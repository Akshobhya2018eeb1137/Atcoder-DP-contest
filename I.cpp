...
int main()
{
	ll n;re(n);vector<long double> p(n);
	FOR(i, 0, n)cin>>p[i];
	// atleast ceil(n/2) should be heads
	vector<vector<long double>> dp(n + 1, vector<long double> (n + 1, 0.0));
	dp[0][0] = 1.0;
	//pr(dp,"\n");
	FOR(i, 1, n + 1)dp[i][0] = p[i - 1] * dp[i - 1][0];
	FOR(i, 1, n + 1)dp[0][i] = (1.0 - p[i - 1]) * dp[0][i - 1];
	///pr(dp, "\n");
	for(ll i = 1;i<n + 1;i++)
	{
		//cout<<"i -===== "<<i<<endl;
		...
		for(ll j = 1;i + j<n + 1;j++)
		{
			//cout<<"j == "<<j<<endl;
			//cout<<",,,,,\n";
			dp[i][j] = p[i + j - 1] * dp[i - 1][j] + (1.0 - p[i + j - 1]) * dp[i][j - 1];
			//cout<<"ch\n";
		}
		//cout<<"na\n";
	} 
