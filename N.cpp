...
int main()
{
	re(n);a.resize(n);re(a);pre.resize(n, 0);
	vvl dp(n + 10, vl (n + 10, inf));
	pre[0] = a[0];for(ll i = 1;i<n;i++)pre[i] = pre[i - 1] + a[i];
	for(ll i = 0;i<n;i++)dp[i][i] = a[i];
	for(ll gap = 1;gap < n;gap++)
	{
		for(ll i = 0;i + gap <n;i++)
		{
			for(ll k = i;k<i + gap;k++)
			{
				dp[i][i + gap] = min(dp[i][i + gap], sum(i, i + gap) + dp[i][k] + dp[k + 1][i + gap]); 
			}
		}		
	}
